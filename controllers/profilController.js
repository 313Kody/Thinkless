...
exports.updatePseudo = async (req, res) => {
  try {
    ...
    if (!pseudo || pseudo.trim().length < 2) {
      return res
        .status(400)
        .json({ message: "Pseudo invalide (2 caractères min.)" });
    }
    const db = getPool();
    await db.execute("UPDATE Utilisateur SET pseudo = ? WHERE id = ?", [
      pseudo.trim(),
      req.user.id,
    ]);
...
exports.updateLocalisation = async (req, res) => {
  try {
    ...
    const db = getPool();
    await db.execute("UPDATE Utilisateur SET localisation = ? WHERE id = ?", [
      localisation || null,
      req.user.id,
    ]);
  ...
  try {
    ...
    if (!regex.test(nouveau_mdp)) {
      return res.status(400).json({
        message:
          "Mot de passe trop faible (12 car. min, majuscule, chiffre, symbole)",
      });
    }
    ...
    await db.execute("UPDATE Utilisateur SET mot_de_passe = ? WHERE id = ?", [
      hash,
      req.user.id,
    ]);
...
exports.quitterEquipe = async (req, res) => {
  try {
    ...
    if (check.length === 0) {
      return res
        .status(400)
        .json({ message: "Vous n'êtes dans aucune équipe" });
    }
    if (check[0].capitaine_id === req.user.id) {
      return res.status(403).json({
        message:
          "Vous êtes capitaine, transférez la capitainerie avant de quitter",
      });
    }
    await db.execute("DELETE FROM EquipeMembre WHERE utilisateur_id = ?", [
      req.user.id,
    ]);
...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    if (!mot_de_passe) {
      return res
        .status(400)
        .json({ message: "Mot de passe requis pour confirmer la suppression" });
    }
    const db = getPool();
    const [rows] = await db.execute(
      "SELECT mot_de_passe FROM Utilisateur WHERE id = ?",
